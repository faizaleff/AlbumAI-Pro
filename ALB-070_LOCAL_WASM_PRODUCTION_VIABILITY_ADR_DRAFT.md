...
Status: **PENDING — WINDOWS HOST EVIDENCE AND SLICE 3 GATES INCOMPLETE**
...
`PENDING` while the real Windows Photoshop/UXP row is missing and the
concurrency, production package, latency, memory, and licensing gates are not
closed.

Automated Node execution, browser WebAssembly support, a simulated Windows
environment, or a macOS result cannot substitute for the missing real Windows
Photoshop/UXP evidence.
...
| Windows Photoshop/UXP | `PENDING` | Mandatory real-host evidence; blocks the final ADR outcome |
| Synthetic package increment | 6,470-byte bundle increase and 1,634-byte release-ZIP increase | Diagnostic cost only; not a production runtime/model budget result |
...
| --- | --- | --- | --- |
| Supported hosts | The production build executes the bounded probe on real supported macOS and Windows Photoshop/UXP hosts | macOS recorded; Windows pending | BLOCKED |
| WASM API compatibility | The chosen runtime path must work with UXP's synchronous `WebAssembly.Module` and `WebAssembly.Instance` constructors; async byte instantiation is not assumed | Synthetic path characterized on macOS; Windows pending | BLOCKED |
| Safety and privacy | Local-only execution, no new network domain, no Photoshop documents opened, bounded safe reports, and no sensitive values retained or logged | Contract and macOS probe pass; Windows confirmation pending | BLOCKED |
| Cancellation and fallback | Cancellation is checked between bounded phases; unsupported, cancelled, stale, or invalid work produces non-publishable unavailable evidence | Contract and macOS probe pass; production-provider design pending | BLOCKED |
...
| Latency budget | Representative preprocessing, cold start, first inference, warm inference, and bounded-batch measurements pass an approved budget on every supported host | Synthetic macOS result only; thresholds and representative evidence pending | BLOCKED |
| Memory budget | WASM allocation, retained references, repeated-run host observation, and recovery after cancellation/failure pass an approved budget on every supported host | Synthetic page known; macOS host reclamation inconclusive; Windows pending | BLOCKED |
...
- [ ] Real Windows Photoshop/UXP production-load and probe evidence is recorded.
- [ ] The Windows run confirms the document count is unchanged and exercises
      cancellation plus repeated-run memory observation.
...
The Slice 2 pull request remains draft and unmerged until the real Windows
Photoshop/UXP procedure is completed and its evidence is reviewed. This Slice 3
preparation does not begin or simulate that test and does not change production
runtime behavior.

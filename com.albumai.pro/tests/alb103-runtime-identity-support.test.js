...
try {
    ...
    check(identitySource.includes('ALBUMAI_RELEASE_STATUS =\n    "CANDIDATE"'), "candidate release status is missing");
    check(identitySource.includes("ALBUMAI_RELEASE_URL = null"), "candidate must not claim a published release URL");
    ...
    check(webpackSource.includes("maxAssetSize: 740 * 1024"), "production bundle budget is not explicit");
    check(webpackSource.includes("maxEntrypointSize: 740 * 1024"), "production entrypoint budget is not explicit");
